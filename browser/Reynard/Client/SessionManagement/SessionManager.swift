...
final class SessionManager {
    ...
    func isDesktopMode(for url: String, tabID: UUID) -> Bool? {
        settings.isDesktopMode(for: url, tabID: tabID)
    }

    func toggleWebsiteMode(for url: String, tabID: UUID) -> WebsiteModeAction? {
        settings.toggleWebsiteMode(for: url, tabID: tabID)
    }
    ...
    func restoreNavigation(for tabID: UUID) -> NavigationAvailability {
        history.restoreState(for: tabID)
    }
    ...
    func navigationAvailability(
        ...
        history.availability(for: tabID, sessionState: sessionState)
    }
    ...
    func recordNavigation(
        ...
        history.record(to: url, for: tabID, sessionState: sessionState)
    }
    ...
    func goBack(
        ...
        history.goBack(for: tabID, sessionState: sessionState)
    }
    ...
    func goForward(
        ...
        history.goForward(for: tabID, sessionState: sessionState)
    }

    func useStoredNavigationHistory(for tabID: UUID) -> NavigationAvailability {
        history.useStoredHistory(for: tabID)
    }
}
